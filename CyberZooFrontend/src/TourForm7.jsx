...
export function TourForm7({habitat5_id, leave_time5, updateFields }) {
    ...
    const fetchHabitats = async () => {
        try {
            const response = await fetch('http://127.0.0.1:8000/api/get_next_habitats/' + habitat5_id + '/' + leave_time5 + '/');
            if (!response.ok) {
            ...
            if (data.length > 0) {
                ...
                updateFields({ habitat6_id: String(data[0].id) });
            }
        } catch (error) {
    ...
    const handleHabitatChange = (event) => {
        ...
        updateFields({ habitat6_id: event.target.value });
    };

    const handleLeaveTimeChange = (event) => {
        setLeaveTime(event.target.value);
        updateFields({ end_time: event.target.value });
    };

    return (
        <FormWrapper title="Choose Last Habitat">
            {errorMessage ? (
