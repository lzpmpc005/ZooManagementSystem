...
export function TourForm5({habitat3_id, leave_time3, updateFields }) {
    ...
    const fetchHabitats = async () => {
        try {
            const response = await fetch('http://127.0.0.1:8000/api/get_next_habitats/' + habitat3_id + '/' + leave_time3 + '/');
            if (!response.ok) {
            ...
            if (data.length > 0) {
                ...
                updateFields({ habitat4_id: String(data[0].id)  });
            }
        } catch (error) {
    ...
    const handleHabitatChange = (event) => {
        ...
        updateFields({ habitat4_id: event.target.value });
        
    };

    const handleLeaveTimeChange = (event) => {
        setLeaveTime(event.target.value);
        updateFields({ leave_time4: event.target.value });
    };

    return (
        <FormWrapper title="Choose Habitat 4">
            {errorMessage ? (
